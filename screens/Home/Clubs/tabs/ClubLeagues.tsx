...
const ClubLeagues: React.FC<ClubLeaguesProps> = ({
  clubId,
  isMember,
  canManage,
}) => {
  ...
  const [leagues, setLeagues] = useState<NormalizedCompetition[]>([]);
  ...
  const fetchClubLeagues = useCallback(async () => {
    ...
    try {
      ...
      console.error("Error fetching club leagues:", e);
    } finally {
      setLoading(false);
    }
  }, [clubId]);
  ...
  const visibleLeagues = useMemo(
    () =>
      isMember
        ? leagues
        : leagues.filter((l) => l.privacy?.toLowerCase() !== "private"),
    [leagues, isMember],
  );

  const sorted = useMemo(
    () => sortLeaguesByEndDate(visibleLeagues),
    [visibleLeagues],
  );
  ...
  const renderContent = useMemo(() => {
    ...
    if (sorted.length === 0) {
      return (
        <NoActivityText>
          No leagues in this club yet. Create one to get started!
        </NoActivityText>
      );
    }
    return (
  ...
  return (
    <Container>
      {canManage && (
        <HeaderRow>
          <CreateButton onPress={() => setAddLeagueModalVisible(true)}>
            <CreateButtonText>+ Create League</CreateButtonText>
          </CreateButton>
        </HeaderRow>
      )}
